...
def load_state():
    try:
        # Update the file path to point to the SD card
        with open('/sd/store.ini', 'r') as f:
            data = json.load(f)
            ...
            skip_valves =data["skip_valves"]
            return last_execution_time, work, interval_seconds, NUMBER_OF_WORK, batch_size, save_water_time_seconds,skip_valves
    except (OSError, ValueError, IndexError):
        print("Error loading state.")
        # Return default values in case of error
        return 1, 1, [3600, 3600, 3600], 3, 1, 3600,[]

def save_state(last_execution_time, work, interval_seconds, NUMBER_OF_WORK, batch_size, save_water_time_seconds,skip_valves):
    data = {
...
skip_valves_setting = 0

def up_button_handler(pin):
    global last_pressed_time, work_index, INTERVAL_SECONDS, NUMBER_OF_WORK1, adjusting_works, select_time_indices, batch_size, save_water_time_index, adjusting_save_water, adjusting_skip_valves,skip_valves_setting
    ...
    if current_time - last_pressed_time > buffer_time * 1000:
        ...
        if setting_mode:
            if adjusting_batch_size:
                ...
                skip_valves_setting += 1
                if skip_valves_setting > NUMBER_OF_WORK1:  # Ensure skip valves don't exceed the number of works
                    skip_valves_setting = 0  # Reset to 0 if it exceeds
                message = ["Adj Skip Valves", f"Skip incr to: {skip_valves_setting}"]
                display_message(lcd, message)
            else:
                if adjusting_works:
                    NUMBER_OF_WORK1 += 1
    ...
    if current_time - last_pressed_time > buffer_time * 1000:
        ...
        if setting_mode:
            if adjusting_batch_size:
                ...
                skip_valves_setting -= 1
                if skip_valves_setting < 0:  # Ensure skip valves don't go below 0
                    skip_valves_setting = NUMBER_OF_WORK1
                message = ["Adj Skip Valves", f"Skip decr to: {skip_valves_setting}"]
                display_message(lcd, message)
            else:
                if adjusting_works:
...
def ok_button_handler(pin):
    global last_pressed_time, last_execution_time, work, interval_seconds, NUMBER_OF_WORK, batch_size_saved, save_water_time_index, adjusting_save_water, adjusting_skip_valves, skip_valves_setting
    ...
    if current_time - last_pressed_time > buffer_time * 1000:
        ...
        if setting_mode:
            if adjusting_works:
                ...
                message = ["Setting end", f"{INTERVAL_SECONDS},{NUMBER_OF_WORK1},{batch_size},{save_water_time_str},{skip_valves_setting}"]
                display_message(lcd, message)
                save_skip_valves=[skip_valves_setting]
                print(message)
                ...
                if all(interval != 0 for interval in INTERVAL_SECONDS) and NUMBER_OF_WORK1 > 0 and batch_size > 0 and save_water_time_seconds > 0:
                    ...
                    save_state(last_execution_time, work, INTERVAL_SECONDS, NUMBER_OF_WORK1, batch_size, save_water_time_seconds,save_skip_valves)
...
def update_skip_valve_from_app(new_value):
    global save_water_time_saved,skip_valves
    try:
        with open('/sd/store.ini', 'r') as f:
            data = json.load(f)
            data["skip_valves"]= new_value 
            with open('/sd/store.ini', 'w') as f:
                json.dump(data, f)
            last_execution_time, work, interval_seconds, NUMBER_OF_WORK, batch_size_saved, save_water_time_saved,skip_valves  = load_state()
            print(f"updated new value{new_value}")
    except OSError:
        print("Error saving state.")
        
def update_rest_time_from_app(new_value):
    global save_water_time_saved
    try:
        with open('/sd/store.ini', 'r') as f:
            data = json.load(f)
            data['save_water_time_seconds']= new_value 
            with open('/sd/store.ini', 'w') as f:
                json.dump(data, f)
            last_execution_time, work, interval_seconds, NUMBER_OF_WORK, batch_size_saved, save_water_time_saved,skip_valves  = load_state()
            print(f"updated new value{new_value}")
...
def update_intervals_from_app(index, new_value):
    global interval_seconds
    try:
        with open('/sd/store.ini', 'r') as f:
            data = json.load(f)
            if 'interval_seconds' in data and 0 <= index < len(data['interval_seconds']):
                data['interval_seconds'][index] = new_value 
                # Move the file cursor to the beginning and truncate the file
                with open('/sd/store.ini', 'w') as f:
                    json.dump(data, f) # Remove any remaining old data in the file
                last_execution_time, work, interval_seconds, NUMBER_OF_WORK, batch_size_saved, save_water_time_saved,skip_valves  = load_state()
                print(f"interval_seconds at index {index} updated to {new_value}.")
            else:
                print("interval_seconds key not found or index out of range.")
    except OSError:
        print("Error saving state.")
 
# Network Task
async def network_task():
    global on_valves,interval_seconds,NUMBER_OF_WORK,current_mode,manual_mode_on_valves,exit_rest_mode,skip_valves
    ...
    async def mqtt_task():
        global on_valves,NUMBER_OF_WORK,interval_seconds,exit_rest_mode,manual_mode_on_valves,skip_valves
        def process_mqtt_data(message):
            global automatic_mode,manual_mode_on_valves,manual_to_automatic,current_mode,exit_rest_mode,skip_valves
            if "valve" in message and "hours" in message or "minutes" in message or "seconds" in message:
                ...
                update_intervals_from_app(valve-1, total_seconds)
            elif "mode" in message:
                mode=message["mode"]
                if mode=="automatic":
                    exit_rest_mode=True
                    
            elif "on_valves" in message and "manual_mode" in message:
            ...
            elif "rest_hours" in message and "rest_minutes" in message:
                hours = message.get("rest_hours", 0)
                minutes = message.get("rest_minutes", 0)
                seconds = message.get("rest_seconds", 0)
                total_seconds = hours * 3600 + minutes * 60 + seconds
                update_rest_time_from_app(total_seconds)
            elif "skipped_valves" in message:
                update_skip_valve=message["skipped_valves"]
                update_skip_valve_from_app(update_skip_valve)
            ...
            if wlan.isconnected():
                try:
                    ...
                    while wlan.isconnected():
                        global on_valves,NUMBER_OF_WORK,interval_seconds,last_execution_time,current_mode,manual_mode_on_valves,skip_valves
...
async def main_task():
    ...
    global last_execution_time, work, interval_seconds, NUMBER_OF_WORK, batch_size_saved, manual_to_automatic, count, rest_time,save_water_time_saved,current_mode,skip_valves
    last_execution_time, work, interval_seconds, NUMBER_OF_WORK, batch_size_saved, save_water_time_saved,skip_valves = load_state()
    rest_time = 0
    previous_work = None
    save_old_data=0
